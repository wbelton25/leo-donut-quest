...
export default class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Buttons
    this._addButton(cx - 60, cy + 32, 'NEW GAME', () => this._startNewGame());
    if (SaveSystem.hasSave()) {
      this._addButton(cx - 60, cy + 50, 'CONTINUE', () => this._continueGame());
    }

    const prompt = txt(this, cx - 60, cy + 70, 'PRESS ANY KEY', {
  ...
  _buildLeaderboard() {
    ...
    const panelX = BASE_WIDTH - 148;
    const panelY = 20;
    const panelW = 140;
    const panelH = BASE_HEIGHT - 36;

    // Panel background
    this.add.rectangle(panelX + panelW / 2, panelY + panelH / 2, panelW, panelH, 0x0a0a1a, 0.9)
      .setStrokeStyle(1, 0x334455);

    txt(this, panelX + panelW / 2, panelY + 8, 'HIGH SCORES', {
      fontSize: '8px', color: '#f5a623',
    }).setOrigin(0.5);

    this.add.rectangle(panelX + panelW / 2, panelY + 20, panelW - 4, 1, 0x334455);
    ...
    if (board.length === 0) {
      ...
      board.forEach((entry, i) => {
        const rowY = panelY + 32 + i * 30;
        const rankColor = i === 0 ? '#ffdd00' : (i === 1 ? '#aaaaaa' : '#996633');
        txt(this, panelX + 8, rowY, `#${i + 1}`, { fontSize: '8px', color: rankColor });
        txt(this, panelX + 8, rowY + 10, `${entry.score} PTS`, { fontSize: '8px', color: '#ffffff' });
        txt(this, panelX + panelW - 8, rowY, `🍩×${entry.donuts}`, {
          fontSize: '8px', color: '#f5a623',
        }).setOrigin(1, 0);
        txt(this, panelX + panelW - 8, rowY + 10, `👥×${entry.partySize}  ${entry.date}`, {
          fontSize: '8px', color: '#778899',
        }).setOrigin(1, 0);
      });

      // Clear scores button
      const clearBg = this.add.rectangle(panelX + panelW / 2, panelY + panelH - 14, 90, 14, 0x1a1a2a)
        .setInteractive({ useHandCursor: true });
      const clearLbl = txt(this, panelX + panelW / 2, panelY + panelH - 14, 'CLEAR SCORES', {
